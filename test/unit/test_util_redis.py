...
class TestRedis(unittest.TestCase):
    ...
    def test_4_clear_cache_for_cluster(self):
        ...
        time.sleep(0.5)
        redis.execute_json_set(client, '/repos/deploy/tamarack/stage', {'value': '1'})       
        redis.execute_json_set(client, '/repos/deploy/tamarack-test/test', {'value': '2'})
        redis.execute_json_set(client, '/repos/deploy/tamarack-test/active', {'value': '3'})       
        time.sleep(0.5)
        redis.clear_cache_for_cluster(client, 'test')
        keys = redis.get_all_keys(client)
        self.assertEqual(len(keys), 2)
        self.assertTrue('/repos/deploy/tamarack/stage' in keys)
        self.assertTrue('/repos/deploy/tamarack-test/active' in keys)
        redis.clear_cache_for_cluster(client, 'stage')
        keys = redis.get_all_keys(client)
